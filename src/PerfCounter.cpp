...
PerfCounter::PerfCounter(const std::string& name, unsigned int type,
                         long long event) {
  _name = name;
  _fd = -1;
...
PerfCounter::PerfCounter() { _fd = -1; }

#ifndef perf_event_open
static int perf_event_open(struct perf_event_attr* hw_event_uptr, pid_t pid,
                           int cpu, int group_fd, unsigned long flags) {
  int ret =
      syscall(__NR_perf_event_open, hw_event_uptr, pid, cpu, group_fd, flags);
  return ret;
}
#endif

bool PerfCounter::init(const std::string& name, unsigned int type,
                       long long event, int group) {
  // already initialized
  ...
  if (fd < 0) {
    fprintf(stderr, "Error opening perf counter %llx %s\n", pe.config,
            name.c_str());
  }
  _name = name;
  _fd = fd;
...
PerfCounter::~PerfCounter() { close(); }

std::string PerfCounter::name() const { return _name; }